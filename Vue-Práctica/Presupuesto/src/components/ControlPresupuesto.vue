<template>
    <div class="dos-columnas">
        <div class="contenedor-grafico">
            <p class="porcentaje">{{ Totalgastos }}</p>
            <CircleProgress
            :percent="Totalgastos"
            :size="250"
            :border-width="30"
...
//Se obtiene el state de App.vue
const props  = defineProps({
 ...
 //En vez de watch también se puede usar: 
//Computed --> la variable se va transformado antes de devolver su valor
const Totalgastos = computed(() => {
  const total = props.gastos.reduce((sum, gasto) => sum + Number(gasto.cantidad), 0)
  return props.presupuesto > 0 ? (total / props.presupuesto) * 100 : 0
})
